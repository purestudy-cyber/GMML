...
data2D_scale = normalize(data2D, 'range');
...
for repeat = 1:repeat_num
    ...
    train_samples = data2D_pca(train_SL(1,:),:)';
    train_labels = train_SL(2,:);
    test_samples = data2D_pca(test_SL(1,:),:)';
...
print_name = ['output\', dataset_name, '_map_PCA','.eps'];
...
if repeat_num == 10
    save_name = ['output\', dataset_name, '_result_PCA'];
